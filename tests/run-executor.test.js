...
import { createExecutionStateMachine, executeGenerationLoop, findMissingPlannedWriteTargets, loadImplementationMemoryContext, sanitizeGeneratedFiles, shouldUseStrictReview } from "../ai-system/core/run-executor.js";
...
describe("Run Executor Core", () => {
    ...
    it("loadImplementationMemoryContext formats memory matches", async () => {
        const memoryMock = {
            ...
            formatForPrompt: (matches) => matches.map((m) => m.content).join("\n")
        };
        const loggerMock = createLogger({ verbose: false });
        const stats = { readFiles: 0, readBytes: 0, implementationMatches: 0 };
        const result = await loadImplementationMemoryContext(memoryMock, "fix bug", { prompt: "fix bug", readFiles: [], writeTargets: [], notes: [] }, stats, loggerMock);
    ...
    it("sanitizeGeneratedFiles permits updates to planned read files", () => {
        ...
        const result = sanitizeGeneratedFiles(files, plan, { max_write_files: 8 }, process.cwd());
    ...
    it("findMissingPlannedWriteTargets reports write targets that were not generated", () => {
        ...
        assert.deepEqual(findMissingPlannedWriteTargets(files, plan), ["src/two.ts"]);
    });
    it("getExecutionBudgetSummary counts previous iterations as retries", () => {
        const artifactState = createArtifactState(process.cwd(), { artifacts: { enabled: true, data_dir: ".ai-system-artifacts" } });
        const state = {
        ...
        const budget = getExecutionBudgetSummary(state, runtime, { max_retries: 3 });
    ...
    it("executeGenerationLoop skips tool checks until every planned write target exists", async () => {
        ...
        let fixerCalls = 0;
        try {
            await fs.writeFile(path.join(repoRoot, "package.json"), JSON.stringify({ name: "loop-gate", private: true, version: "1.0.0" }, null, 2), "utf8");
