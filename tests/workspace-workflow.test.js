...
test("workspace assessment, graph, and checklist are generated deterministically", async () => {
    const rules = { artifacts: { data_dir: ".artifacts" } };
    const workItem = {
...
test("workspace API can create assess run and list work items", async () => {
    ...
    try {
        ...
        assert.equal(run.workItem.graph.nodes.find((node) => node.id === "inspect-1").assignedRunId, run.job.jobId);
        ...
        assert.equal(loaded.workItem.graph.nodes.find((node) => node.id === "inspect-1").status, "completed");
        assert.equal(loaded.workItem.checklist.find((item) => item.id === "inspect-1").status, "passed");
        ...
        assert.ok(events.events.some((event) => event.type === "status"));
        assert.ok(events.events.some((event) => event.type === "run"));
        const list = await requestJson(baseUrl, "GET", `/work-items?cwd=${encodeURIComponent(repoRoot)}`);
        assert.equal(list.ok, true);
        assert.ok(list.workItems.some((item) => item.id === created.workItem.id));
    }
    finally {
...
function createResult(task, cwd, dryRun) {
    return {
        version: 1,
        ok: true,
        status: "completed",
