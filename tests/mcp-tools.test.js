...
describe("MCP tools", () => {
    ...
    test("creates, reads, runs, and cancels work items through service layer", async () => {
        ...
        try {
            ...
            const loaded = await executeMcpTool(ctx, "orchestra_get_work_item", { workItemId: created.workItem.id });
            assert.equal(loaded.ok, true);
            const run = await executeMcpTool(ctx, "orchestra_run_work_item", { workItemId: created.workItem.id, dryRun: true });
            assert.equal(run.ok, true);
            assert.equal(run.jobs[0].workflowProfile, "superpowers");
            const cancelled = await executeMcpTool(ctx, "orchestra_cancel_work_item", { workItemId: created.workItem.id });
