...
test("Phase W0 — workspace artifacts can coexist with run artifacts", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(workItemDir, "work-item.json"), JSON.stringify({ id: "work_test001", title: "Test work item", status: "created" }));
...
test("Phase W0 — docs/WORKSPACE.md exists and contains key domain terms", async () => {
    const workspaceDocPath = path.resolve((typeof import.meta !== "undefined" && import.meta.dirname) ? import.meta.dirname : __dirname, "..", "docs", "WORKSPACE.md");
    ...
    for (const term of requiredTerms) {
        assert.ok(content.includes(term), `WORKSPACE.md must define the domain term: "${term}"`);
    }
