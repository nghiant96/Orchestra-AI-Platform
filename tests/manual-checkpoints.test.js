...
test("loadEditablePlanCheckpoint reads user-edited plan.json safely", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(artifactPath, "plan.json"), JSON.stringify({
            normalizedPlan: {
...
test("loadEditableContextCheckpoint reloads edited context files", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(artifactPath, "context.json"), JSON.stringify({ savedFiles: ["src/context.ts", "../unsafe.ts"] }, null, 2), "utf8");
