...
describe("work item lesson exporter", () => {
  test("generates redacted success and failure lessons", async () => {
    ...
    try {
      ...
      const persisted = await generateAndPersistWorkItemLesson(tmp, { artifacts: { data_dir: ".artifacts" } } as any, workItem, jobs);
...
function makeWorkItem(status: WorkItem["status"]): WorkItem {
