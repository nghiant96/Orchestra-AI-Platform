...
test("runToolChecks auto-detects npm scripts and stores structured command results", async () => {
    ...
    const changedFiles = [{ path: "src/example.ts", content: "export const value = 1;\n" }];
    try {
...
test("runToolChecks supports changed-file placeholders in configured script args", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(tempDir, "scripts", "print-args.js"), "console.log(process.argv.slice(2).join('|')); process.exit(0);\n", "utf8");
...
test("runToolChecks auto-detects scoped lint/test scripts for changed files", async () => {
    ...
    const changedFiles = [{ path: "src/example.test.ts", content: "export const value = 1;\n" }];
    try {
        await fs.mkdir(path.join(tempDir, "scripts"), { recursive: true });
        await fs.writeFile(path.join(tempDir, "pnpm-lock.yaml"), "lockfileVersion: '9.0'\n", "utf8");
        await fs.writeFile(path.join(tempDir, "package.json"), JSON.stringify({
...
test("runToolChecks scopes lint/test to a single changed workspace package", async () => {
    ...
    const changedFiles = [{ path: "packages/web/src/example.ts", content: "export const value = 1;\n" }];
    try {
        ...
        await fs.writeFile(path.join(tempDir, "packages/web/scripts/print-cwd.js"), "console.log(process.cwd()); process.exit(0);\n", "utf8");
        const rules = createRules({
            tools: {
                enabled: true,
                json_validation: true,
...
test("runToolChecks prefers the changed package lint script over the root lint script", async () => {
    ...
    const changedFiles = [{ path: "dashboard/src/App.tsx", content: "export const App = () => null;\n" }];
    try {
        await fs.mkdir(path.join(tempDir, "scripts"), { recursive: true });
        await fs.writeFile(path.join(tempDir, "pnpm-lock.yaml"), "lockfileVersion: '9.0'\n", "utf8");
        ...
        await fs.writeFile(path.join(tempDir, "dashboard/package.json"), JSON.stringify({
            name: "dashboard",
            private: true,
            scripts: {
...
test("runToolChecks scopes generic package eslint scripts to changed files", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(tempDir, "dashboard/bin/pnpm"), "#!/usr/bin/env node\nconsole.log(process.argv.slice(2).join('|')); process.exit(0);\n", "utf8");
...
test("runToolChecks scopes typecheck to a single changed workspace package when it has a tsconfig", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(tempDir, "pnpm-lock.yaml"), "lockfileVersion: '9.0'\n", "utf8");
        await fs.writeFile(path.join(tempDir, "package.json"), JSON.stringify({ name: "workspace-root", private: true }, null, 2), "utf8");
        await fs.writeFile(path.join(tempDir, "packages/web/package.json"), JSON.stringify({ name: "@workspace/web", private: true }, null, 2), "utf8");
        await fs.writeFile(path.join(tempDir, "packages/web/tsconfig.json"), JSON.stringify({ compilerOptions: { noEmit: true } }, null, 2), "utf8");
        await fs.writeFile(path.join(tempDir, "node_modules/typescript/bin/tsc"), "console.log(process.cwd()); console.log(process.argv.slice(2).join('|')); process.exit(0);\n", "utf8");
        const rules = createRules({
            tools: {
                enabled: true,
                json_validation: true,
...
test("runToolChecks prefers a changed package tsconfig over the root typecheck script", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(tempDir, "dashboard/package.json"), JSON.stringify({ name: "dashboard", private: true }, null, 2), "utf8");
        await fs.writeFile(path.join(tempDir, "dashboard/tsconfig.json"), JSON.stringify({ compilerOptions: { noEmit: true } }, null, 2), "utf8");
        await fs.writeFile(path.join(tempDir, "node_modules/typescript/bin/tsc"), "console.log(process.cwd()); console.log(process.argv.slice(2).join('|')); process.exit(0);\n", "utf8");
        const summary = await runToolChecks({
            repoRoot: tempDir,
            changedFiles,
...
test("runToolChecks uses pnpm workspace filters when changes span multiple packages", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(tempDir, "packages/web/scripts/print-package.js"), "console.log(process.cwd()); process.exit(0);\n", "utf8");
        await fs.writeFile(path.join(tempDir, "packages/api/scripts/print-package.js"), "console.log(process.cwd()); process.exit(0);\n", "utf8");
        const rules = createRules({
            tools: {
                enabled: true,
                json_validation: true,
...
test("runToolChecks supports clean-env sandbox mode with explicit env passthrough", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(tempDir, "scripts", "print-env.js"), "console.log(process.env.AI_SYSTEM_TOOL_SANDBOX_SECRET ?? 'missing'); process.exit(0);\n", "utf8");
        const rules = createRules({
            tools: {
                enabled: true,
                json_validation: true,
...
test("runToolChecks skips docker sandbox when docker is unavailable", async () => {
    ...
    try {
        ...
        await fs.writeFile(path.join(tempDir, "package.json"), JSON.stringify({ name: "docker-missing", scripts: { lint: "echo lint" } }, null, 2), "utf8");
...
test("runToolChecks auto-builds missing docker image when enabled", async () => {
    ...
    try {
        await fs.mkdir(path.join(tempDir, "bin"), { recursive: true });
        await fs.writeFile(path.join(tempDir, "package.json"), JSON.stringify({ name: "docker-autobuild", scripts: { lint: "echo lint" } }, null, 2), "utf8");
        await fs.writeFile(path.join(tempDir, "pyproject.toml"), "[project]\nname = \"docker-autobuild\"\n", "utf8");
        await fs.writeFile(path.join(tempDir, "Dockerfile.tools"), "FROM scratch\n", "utf8");
        await fs.writeFile(path.join(tempDir, "bin/docker"), [
            "#!/usr/bin/env node",
            "const fs = require('fs');",
