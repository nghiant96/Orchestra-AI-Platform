...
import { createJob, createSyncRun, listJobs, getJob, cancelJob, approveJob, getJobFileContent, parseWorkflowMode, isPathWithinRoot, mapRunSummaryToQueueJob } from "../ai-system/jobs/job-service.js";
...
describe("JobService", () => {
    ...
    test("createJob rejects missing task", async () => {
        await assert.rejects(createJob(ctx(), { task: "", cwd: tmpDir }), /Missing task/);
    });
    ...
    test("approveJob approves a pending job", async () => {
        ...
        queue.get = async (id) => id === jobId ? { jobId, status: "waiting_for_approval", cwd: tmpDir, task: "approve test", dryRun: true } : origGet(id);
    ...
    test("createSyncRun runs synchronously", async () => {
        const c = {
            ...ctx(),
            runNow: async (input) => createResult({ task: input.task, cwd: input.cwd, ok: true })
        };
    ...
    test("mapRunSummaryToQueueJob maps all statuses", () => {
        ...
        assert.equal(mapRunSummaryToQueueJob({ ...baseRun, status: "paused_after_generate" }, "/repo").status, "waiting_for_approval");
