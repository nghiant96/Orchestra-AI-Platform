...
test("FileBackedJobQueue retention removes old job records and keeps recent records", async () => {
    ...
    const queue = new FileBackedJobQueue(jobsDir, async ({ task, cwd, dryRun }) => ({
        version: 1,
        ok: true,
        status: "completed",
        dryRun,
    ...
    try {
        ...
        await fs.writeFile(path.join(jobsDir, "old.json"), JSON.stringify(createQueueJob("old", "2020-01-01T00:00:00.000Z")), "utf8");
        await fs.writeFile(path.join(jobsDir, "recent.json"), JSON.stringify(createQueueJob("recent", new Date().toISOString())), "utf8");
...
function createAuditEvent(details) {
    return {
        version: 1,
        id: "evt-1",
...
function createQueueJob(jobId, createdAt) {
    return {
        version: 1,
        jobId,
        status: "completed",
