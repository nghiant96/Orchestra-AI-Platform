...
import { createArtifactState, persistContextArtifacts, persistIterationArtifacts, persistPlanArtifacts, persistRunState } from "../ai-system/core/artifacts.js";
...
test("Orchestrator.resume completes a paused plan by running generator and reviewer", async () => {
    ...
    try {
        ...
        assert.equal(await fs.readFile(path.join(repo.repoRoot, "src/result.ts"), "utf8"), "export const output = 'from-generator';\n");
...
test("Orchestrator.resume re-enters the fixer loop when a paused generation has blocking issues", async () => {
    ...
    try {
        ...
        assert.equal(await fs.readFile(path.join(repo.repoRoot, "src/result.ts"), "utf8"), "export const output = 'fixed-after-resume';\n");
...
async function createTestRepo(overrides = {}) {
    ...
    return {
        ...
        readSavedRunState: async () => {
            ...
            return JSON.parse(await fs.readFile(path.join(statePath, latestRunDir, "run-state.json"), "utf8"));
        }
    };
}
