...
test("loadArtifactCandidate and applyArtifactCandidate load and apply the latest iteration artifact", async () => {
    ...
    try {
        ...
        const artifactTarget = state.latestIterationPath;
        ...
        const applyEvent = JSON.parse(await fs.readFile(result.applyEventPath, "utf8"));
        const timeline = await fs.readFile(path.join(state.runDir, "timeline.jsonl"), "utf8");
        const artifactIndex = JSON.parse(await fs.readFile(path.join(state.runDir, "artifact-index.json"), "utf8"));
