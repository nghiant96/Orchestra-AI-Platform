...
describe("Phase 2 worker CLI", () => {
    ...
    test("worker sends mutation checkpoint before writing a file", async () => {
        ...
        try {
            ...
            const created = await requestJson(baseUrl, "POST", "/jobs", { task: "worker:write-file output.txt::content with sk-test12345678901234567890", cwd: repoRoot, dryRun: true }, 202, operatorHeaders);
            const summary = await runWorkerRuntime(loadWorkerRuntimeConfig({
                cwd: repoRoot,
                serverUrl: baseUrl,
                workerToken: "worker-token",
