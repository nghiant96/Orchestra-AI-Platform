...
import { buildRoutingDecision, chooseProfile, resolveRoleProviders } from "../ai-system/core/provider-router.js";
const DEFAULT_RULES = {
    providers: {
...
describe("Provider Router Core", () => {
    ...
    it("resolves role providers correctly for implementation stage with docs-only signal", () => {
        const roleProviders = resolveRoleProviders(DEFAULT_RULES, "balanced", "implementation", null, [{ name: "plan:docs-only", matched: true }]);
    ...
    it("buildRoutingDecision parses file system repo signals", async () => {
        mock.method(fs, "access", async (pathLike) => {
