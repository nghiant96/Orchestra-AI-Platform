...
test("server jobs API enqueues, completes, lists, and returns stable JSON", async () => {
    ...
    try {
        ...
        assert.equal(listed.jobs.some((job) => job.jobId === created.jobId), true);
    }
    finally {
...
test("server cancels queued jobs", async () => {
    ...
    let releaseFirstJob = () => { };
    const releaseFirstJobSignal = new Promise((resolve) => {
...
test("queue approval mode follows skip_approval config", () => {
    assert.deepEqual(resolveQueueRunApprovalMode({ skip_approval: true }), {
        interactive: false,
        pauseAfterPlan: false
    });
    assert.deepEqual(resolveQueueRunApprovalMode({ skip_approval: false }), {
        interactive: true,
        pauseAfterPlan: true
    });
    assert.deepEqual(resolveQueueRunApprovalMode({}), {
        interactive: true,
        pauseAfterPlan: true
    });
});
...
test("project registry and audit log expose multi-project operations", async () => {
    ...
    try {
        ...
        assert.ok(projects.projects.some((project) => project.cwd === repoRoot && project.queueDir.includes(".ai-system-server")));
        ...
        assert.ok(audit.events.some((event) => event.action === "job.create" && event.actor.id === "tester"));
    }
    finally {
        await closeServer(server);
        await cleanupDir(repoRoot);
...
test("workspace registration persists and rehydrates on restart", async () => {
    ...
    try {
        ...
        const projects = await requestJson(baseUrl, "GET", "/projects");
        assert.ok(projects.projects.some((project) => project.cwd === canonicalOtherRoot));
    }
    finally {
...
test("server smoke covers health projects jobs stats lessons and audit", async () => {
    ...
    try {
        ...
        const projects = await requestJson(baseUrl, "GET", "/projects");
        assert.equal(projects.ok, true);
        assert.ok(projects.projects.some((project) => project.cwd === repoRoot));
        const created = await requestJson(baseUrl, "POST", "/jobs", { task: "smoke dry-run job", cwd: repoRoot, dryRun: true }, undefined, {
            "x-ai-system-role": "operator",
        ...
        assert.ok(jobs.jobs.some((job) => job.jobId === created.jobId));
        ...
        await requestJson(baseUrl, "POST", "/lessons", { cwd: repoRoot, title: "Smoke lesson", body: "Smoke tests must cover operator-visible server workflows." }, 201, {
            "x-ai-system-role": "operator",
            "x-ai-system-actor": "smoke"
        });
        const lessonsAfter = await requestJson(baseUrl, "GET", `/lessons?cwd=${encodedCwd}`);
        assert.ok(lessonsAfter.lessons.some((lesson) => lesson.title === "Smoke lesson"));
        const audit = await requestJson(baseUrl, "GET", "/audit");
        assert.ok(audit.events.some((event) => event.action === "job.create" && event.actor.id === "smoke"));
        assert.ok(audit.events.some((event) => event.action === "lesson.create" && event.actor.id === "smoke"));
    }
    finally {
        await closeServer(server);
        await cleanupDir(repoRoot);
...
test("jobs API filters queue records by requested project cwd", async () => {
    ...
    try {
        ...
        assert.deepEqual(projectA.jobs.map((job) => job.cwd), [canonicalRepoRoot]);
        const projectB = await requestJson(baseUrl, "GET", `/jobs?cwd=${encodeURIComponent(otherRoot)}`);
        assert.deepEqual(projectB.jobs.map((job) => job.cwd), [canonicalOtherRoot]);
...
async function requestJsonMaybe(baseUrl, method, pathname, body) {
...
function createResult({ task, cwd, dryRun, ok }) {
    return {
        version: 1,
        ok,
