...
test("symbol parser registry selects extensible parsers and plain-text fallback", async () => {
    ...
    const rustRanges = await detectSymbolRanges("src/lib.rs", ["pub fn validate_token(token: &str) -> bool {", "  !token.is_empty()", "}", "", "pub struct Session {}"].join("\n"));
...
test("VectorIndex prefers code files over roadmap docs for implementation queries", async () => {
    const repoRoot = await createTempRepo({
        "ai-system/core/tool-executor.ts": "export function buildToolInvocation() { return 'docker sandbox passthrough env'; }\n",
        "tasks/roadmap-v2.md": "# Roadmap\nImprove docker sandbox passthrough and semantic vector search for future work.\n",
        "README.md": "Docker sandbox passthrough notes and high level setup instructions.\n"
    });
    try {
        const rules = createRules();
        const index = new VectorIndex({
