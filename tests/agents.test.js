...
describe("Agents Core", () => {
    ...
    it("GeneratorAgent constructs proper prompts for candidate generation", async () => {
        ...
        const result = await generator.generateCode("fix bug", plan, [{ path: "src/index.ts", content: "" }], // contextFiles
        "/mock");
        assert.equal(mockProvider.runJson.mock.callCount(), 1);
        const callArgs = mockProvider.runJson.mock.calls[0].arguments[0];
    ...
    it("GeneratorAgent injects refactor examples and rejects files outside planned scope", async () => {
        ...
        await assert.rejects(() => generator.generateCode("refactor service", plan, [{ path: "src/index.ts", content: "" }], "/mock"), /outside the planned scope/);
        const callArgs = mockProvider.runJson.mock.calls[0].arguments[0];
    ...
    it("ReviewerAgent records dropped invalid-path issues as validation notes", async () => {
        ...
        const result = await reviewer.reviewCode("review candidate", null, false, [{ path: "src/index.ts", content: "before" }], [{ path: "src/index.ts", content: "after" }], [], [], "/mock");
    ...
    it("ReviewerAgent sends plan context and strict review instructions for high-risk reviews", async () => {
        ...
        await reviewer.reviewCode("review high-risk candidate", plan, true, [{ path: "src/auth.ts", content: "before" }], [{ path: "src/auth.ts", content: "after" }], [], [], "/mock");
        const callArgs = mockProvider.runJson.mock.calls[0].arguments[0];
