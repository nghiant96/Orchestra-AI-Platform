...
describe("Phase 2 worker CLI", () => {
  ...
  test("worker registers, claims, uploads redacted logs, and completes dummy jobs", async () => {
    ...
    try {
      ...
      const created = await requestJson(
        ...
        "POST",
        "/jobs",
        { task: "dummy job with secret sk-test12345678901234567890", cwd: repoRoot, dryRun: true },
        202,
        operatorHeaders
      );
  ...
  test("worker sends mutation checkpoint before writing a file", async () => {
    ...
    try {
      ...
      assert.ok(Array.isArray(job.workerLogs));
      const checkpointIndex = job.workerLogs.findIndex((line: string) => line.includes("checkpointing filesystem mutation"));
      const writeIndex = job.workerLogs.findIndex((line: string) => line.includes("writing output.txt"));
...
function createResult(task: string, cwd: string) {
  return {
    version: 1,
    ok: true,
