...
describe("WorkItem Store", () => {
    ...
    test("normalizes missing fields", async () => {
        ...
        await fs.writeFile(path.join(workDir, "work-item.json"), JSON.stringify({ id: "work-incomplete", title: "Incomplete" }), "utf8");
    ...
    test("rejects unsafe work item ids", async () => {
        await assert.rejects(() => store.create({ id: "../escape", title: "Escape", projectId: "p1" }), /Invalid work item id/);
        await assert.rejects(() => store.load("../escape"), /Invalid work item id/);
        await assert.rejects(() => store.load("work-../../escape"), /Invalid work item id/);
    });
