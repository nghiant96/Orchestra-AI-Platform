...
describe("Token Policy", () => {
    ...
    test("hermes cannot access worker routes", () => {
        assert.equal(canAccessRoute("hermes", "/workers"), false);
        assert.equal(canAccessRoute("hermes", "/jobs/j1/complete"), false);
    });
    test("worker cannot access dashboard APIs", () => {
        assert.equal(canAccessRoute("worker", "/config", "GET"), false);
        assert.equal(canAccessRoute("worker", "/audit", "GET"), false);
        assert.equal(canAccessRoute("worker", "/stats", "GET"), false);
        assert.equal(canAccessRoute("worker", "/workers", "GET"), false);
        assert.equal(canAccessRoute("worker", "/workers/w1", "GET"), false);
    });
    test("worker can access worker routes", () => {
        assert.equal(canAccessRoute("worker", "/workers", "POST"), true);
        assert.equal(canAccessRoute("worker", "/workers/w1/heartbeat", "POST"), true);
        assert.equal(canAccessRoute("worker", "/workers/w1/jobs/claim", "POST"), true);
        assert.equal(canAccessRoute("worker", "/jobs/j1/complete", "POST"), true);
    });
    test("hermes can access work item and job APIs", () => {
        assert.equal(canAccessRoute("hermes", "/work-items"), true);
        assert.equal(canAccessRoute("hermes", "/jobs"), true);
        assert.equal(canAccessRoute("hermes", "/health"), true);
    });
});
