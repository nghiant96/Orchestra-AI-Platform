...
const readyServers = new WeakSet();
/**
...
export async function waitForHttpReady(baseUrl, readyPath = "/health", extraHeaders = {}) {
    const maxAttempts = 20;
    for (let i = 0; i < maxAttempts; i++) {
        try {
            const statusCode = await new Promise((resolve, reject) => {
                const req = http.get(new URL(readyPath, baseUrl), {
 ...
 */
export async function requestJson(baseUrl, method, pathname, body, expectedStatus, extraHeaders = {}) {
