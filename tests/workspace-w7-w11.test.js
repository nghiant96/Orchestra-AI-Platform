...
test("W7-W11 workspace helpers cover commit, PR, inbox, CI, and scheduling", async () => {
    ...
    try {
        ...
        const store = new WorkStore(repoRoot, { artifacts: { data_dir: ".ai-system-artifacts" } });
        ...
        const ci = await watchCiForWorkItem({ ...workItem, ci: { status: "failing", failingChecks: ["pnpm test"] } }, repoRoot);
        assert.equal(ci.repairNeeded, true);
        assert.match(proposeCiRepairTask(workItem, ci), /Fix CI/);
        const plan = scheduleWorkItems([
            { ...workItem, id: "work-a", status: "executing", branch: "work/a", worktreePath: "/tmp/a" },
            { ...workItem, id: "work-b", status: "created", branch: "work/a", worktreePath: "/tmp/a" }
        ]);
        ...
        const tieredPlan = scheduleWorkItems([
            { ...workItem, id: "w-feature", type: "feature", status: "created", branch: "w1", worktreePath: "/w1" },
            { ...workItem, id: "w-docs", type: "docs", status: "created", branch: "w2", worktreePath: "/w2" },
            { ...workItem, id: "w-refactor", type: "refactor", status: "created", branch: "w3", worktreePath: "/w3" },
            { ...workItem, id: "w-test", type: "test", status: "created", branch: "w4", worktreePath: "/w4" },
            { ...workItem, id: "w-bugfix", type: "bugfix", status: "created", branch: "w5", worktreePath: "/w5" }
        ]);
        ...
        const cappedPlan = scheduleWorkItems([
            { ...workItem, id: "w-a", status: "created", branch: "b1", worktreePath: "/b1" },
            { ...workItem, id: "w-b", status: "created", branch: "b2", worktreePath: "/b2" },
            { ...workItem, id: "w-c", status: "created", branch: "b3", worktreePath: "/b3" }
        ], { maxParallel: 2 });
        ...
        const statusPlan = scheduleWorkItems([
            { ...workItem, id: "w-fail", status: "failed", branch: "f1", worktreePath: "/f1" },
            { ...workItem, id: "w-cancel", status: "cancelled", branch: "f2", worktreePath: "/f2" },
            { ...workItem, id: "w-ok", status: "created", branch: "f3", worktreePath: "/f3" }
        ]);
