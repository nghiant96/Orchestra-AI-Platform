...
export async function registerWorker(ctx, input) {
...
export async function updateHeartbeat(ctx, workerId, input) {
...
export async function setWorkerStatus(ctx, workerId, action) {
    const worker = await ctx.store.load(workerId);
    if (!worker) {
        throw new WorkerServiceError("Worker not found", 404);
    }
...
export async function claimJob(ctx, workerId) {
    ...
    const activeLeaseJobIds = new Set(jobs.filter((j) => j.lease && new Date(j.lease.expiresAt).getTime() > Date.now()).map((j) => j.jobId));
    for (const job of jobs) {
        if (job.status !== "queued")
            continue;
        if (job.lease && new Date(job.lease.expiresAt).getTime() > Date.now())
            continue;
        const currentAttempt = job.attempt ?? 0;
        if (currentAttempt >= MAX_ATTEMPTS)
            continue;
        if (!workerMatchesJobSelector(worker, job))
            continue;
        if (!(await isPathWithinWorkspaceRoots(job.cwd, worker.workspaceRoots)))
            continue;
        const now = new Date();
        const lease = {
            workerId,
...
export async function appendWorkerLogs(ctx, workerId, jobId, leaseId, logs) {
...
export async function recoverStalledJob(ctx, jobId) {
...
export async function detectStaleLeases(ctx) {
...
export async function completeJob(ctx, workerId, jobId, leaseId, result) {
...
export async function failJob(ctx, workerId, jobId, leaseId, errorMessage, result) {
...
function workerMatchesJobSelector(worker, job) {
...
async function resolveComparablePath(candidate) {
    try {
