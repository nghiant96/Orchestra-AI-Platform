...
export async function runSetupWizard({ cwd, configPath, explicitGlobalConfigPath, ignoreProjectConfig }) {
    ...
    const providerChoices = ["auto", "codex-cli", "agy-cli", "claude-cli"];
    try {
        console.log("");
        console.log("Setup");
        console.log(`- repo: ${cwd}`);
        console.log(`- config: ${explicitGlobalConfigPath ?? inspection.configPath ?? path.join(cwd, ".ai-system.json")}${explicitGlobalConfigPath ? " (global)" : ""}`);
        console.log(`- current providers: planner=${inspection.effectiveRules.providers.planner.type}, reviewer=${inspection.effectiveRules.providers.reviewer.type}, generator=${inspection.effectiveRules.providers.generator.type}, fixer=${inspection.effectiveRules.providers.fixer.type}`);
        ...
        const memoryBackend = await promptForChoice({
            ...
            defaultValue: inspection.effectiveRules.memory?.backend === "openmemory" || inspection.effectiveRules.memory?.backend === "local-file"
        ...
        for (const [toolName, selection] of Object.entries(toolSelections)) {
            console.log(`  - ${toolName}: mode=${selection.mode}${selection.script ? `, script=${selection.script}` : ""}${selection.appendChangedFiles ? ", changed-files=true" : ""}`);
        }
...
function providerChoiceDescriptions() {
    return {
        ...
        "agy-cli": "Useful for planning or review when you want Antigravity CLI (agy) in the loop.",
        "claude-cli": "Useful for review or planning when Claude CLI is available on the machine."
    };
}
async function promptForToolSetup(rl, inspection, toolName) {
...
function currentSetupProviderChoice(inspection, role) {
...
function currentToolMode(currentConfig) {
...
function currentToolScriptName(currentConfig, toolName) {
    if (!currentConfig || typeof currentConfig !== "object" || Array.isArray(currentConfig)) {
        return toolName;
    }
    const candidate = currentConfig;
...
async function promptForChoice({ rl, label, choices, defaultValue, descriptions }) {
    console.log("");
    console.log(label);
...
async function promptForInput({ rl, label, defaultValue, allowEmpty = false }) {
    while (true) {
