import { formatDuration, formatExecutionBudget, summarizeToolResults, summarizeIssueCountsFromIssues, formatCost } from "./shared.js";
export function printRecentRunSummary(summary) {
    ...
    if (summary.runState.providers) {
        console.log(`- providers: planner=${summary.runState.providers.planner}, reviewer=${summary.runState.providers.reviewer}, generator=${summary.runState.providers.generator}, fixer=${summary.runState.providers.fixer}`);
    }
    if (summary.routing.planning || summary.routing.implementation) {
        console.log("- routing:");
        if (summary.routing.planning) {
            console.log(`  - planning: profile=${summary.routing.planning.profile}, enabled=${summary.routing.planning.enabled}, reason=${summary.routing.planning.reason}`);
        }
        if (summary.routing.implementation) {
            console.log(`  - implementation: profile=${summary.routing.implementation.profile}, enabled=${summary.routing.implementation.enabled}, reason=${summary.routing.implementation.reason}`);
        }
    }
    ...
    if (execution) {
        ...
        console.log(`- failure class: ${execution.failure ? `${execution.failure.class} (${execution.failure.reason})` : "none"}`);
        if (execution.budget) {
    ...
    if (summary.artifactIndex?.applyEventCount) {
        console.log(`- apply events: count=${summary.artifactIndex.applyEventCount}, latest=${summary.artifactIndex.latestApplyEventPath ?? "(unknown)"}${summary.artifactIndex.lastAppliedAt ? ` at ${summary.artifactIndex.lastAppliedAt}` : ""}`);
    }
    if (summary.artifactIndex?.runPath) {
        console.log(`- artifact run: ${summary.artifactIndex.runPath}`);
    }
}
export function printRunList(runs, repoRoot) {
    console.log("");
    ...
    for (const run of runs) {
        ...
        console.log(`- ${run.runName}: status=${run.status}, iterations=${run.iterationCount}, updated=${run.updatedAt ?? "(unknown)"}`);
        console.log(`  task: ${run.task || "(unknown)"}`);
        console.log(`  state: ${run.statePath}`);
        if (execution) {
            console.log(`  execution: total=${formatDuration(execution.totalDurationMs)}, cost=${formatCost(execution.budget?.totalCostUnits || 0)}, failure=${execution.failure ? execution.failure.class : "none"}`);
        }
        if (run.applyEventCount) {
            console.log(`  apply: count=${run.applyEventCount}, latest=${run.latestApplyEventPath ?? "(unknown)"}${run.lastAppliedAt ? ` at ${run.lastAppliedAt}` : ""}`);
        }
        console.log(`  files: ${run.latestFiles.join(", ") || "(none)"}`);
    }
}
export function printArtifactApplyResult(result) {
    console.log("");
