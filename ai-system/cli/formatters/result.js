...
export function printResult(result) {
    ...
    console.log(`- providers: planner=${result.providers?.planner}, reviewer=${result.providers?.reviewer}, generator=${result.providers?.generator}, fixer=${result.providers?.fixer}`);
    console.log(`- memory: backend=${result.memory?.backend}, planning_matches=${result.memory?.planningMatches ?? 0}, implementation_matches=${result.memory?.implementationMatches ?? 0}, stored=${result.memory?.stored}`);
    if (result.execution) {
        console.log(`- execution: total=${formatDuration(result.execution.totalDurationMs)}`);
        console.log(`- execution stage: current=${result.execution.currentStage ?? "none"}, terminal=${result.execution.terminalStage ?? "none"}`);
        console.log(`- failure class: ${result.execution.failure ? `${result.execution.failure.class} (${result.execution.failure.reason})` : "none"}`);
        ...
        if ((result.execution.providerMetrics ?? []).length > 0) {
            ...
            for (const metric of result.execution.providerMetrics ?? []) {
                console.log(`  - ${metric.role}/${metric.provider}: duration=${formatDuration(metric.totalDurationMs)}, cost=${metric.estimatedCostUnits.toFixed(2)}, stages=${metric.stages.join(",")}`);
            }
        }
    }
    if ((result.latestToolResults ?? []).length > 0) {
        ...
        for (const tool of result.latestToolResults ?? []) {
            console.log(`  - ${tool.name}: ${tool.skipped ? "skipped" : tool.ok ? "passed" : "failed"} (${tool.durationMs}ms)${tool.scope ? ` [scope=${tool.scope}]` : ""}${tool.sandboxMode ? ` [sandbox=${tool.sandboxMode}]` : ""}${tool.workingDirectory ? ` [cwd=${tool.workingDirectory}]` : ""}${tool.command ? ` -> ${tool.command}${tool.args && tool.args.length > 0 ? ` ${tool.args.join(" ")}` : ""}` : ""}`);
        }
    }
    ...
    console.log(`- issues: high=${result.issueCounts?.high ?? 0}, medium=${result.issueCounts?.medium ?? 0}, low=${result.issueCounts?.low ?? 0}`);
    ...
    if (iterations.length > 0) {
        ...
        for (const iteration of iterations) {
            ...
            const toolSuffix = (iteration.toolResults ?? []).length > 0
    ...
    if (!result.ok && result.status?.startsWith("paused_")) {
        ...
        const blockingIssues = (result.finalIssues ?? []).filter((issue) => issue.severity === "high" || issue.severity === "medium");
...
export function printRetryResult(target, stage, result) {
    console.log("");
    console.log("Retry");
...
export function printFixChecksPreparation(preparation) {
    console.log("");
    console.log("Fix Checks");
    console.log(`- repo: ${preparation.repoRoot}`);
    console.log(`- config: ${preparation.configPath ?? "(default rules)"}`);
    console.log(`- providers: planner=${preparation.providers.planner}, reviewer=${preparation.providers.reviewer}, generator=${preparation.providers.generator}, fixer=${preparation.providers.fixer}`);
    console.log(`- failing checks: ${preparation.failingChecks.map((entry) => entry.name).join(", ") || "(none)"}`);
    console.log(`- tool issue counts: high=${preparation.issueCounts.high}, medium=${preparation.issueCounts.medium}, low=${preparation.issueCounts.low}`);
    if (preparation.fileHints.length > 0) {
        console.log(`- file hints: ${preparation.fileHints.join(", ")}`);
    }
}
export function printFixFromRunPreparation(preparation) {
    console.log("");
    ...
    console.log(`- previous providers: planner=${preparation.providers.planner}, reviewer=${preparation.providers.reviewer}, generator=${preparation.providers.generator}, fixer=${preparation.providers.fixer}`);
