...
export function printConfigShow(inspection) {
    console.log("");
    console.log("Config");
    ...
    console.log(`- effective providers: planner=${inspection.effectiveRules.providers.planner.type}, reviewer=${inspection.effectiveRules.providers.reviewer.type}, generator=${inspection.effectiveRules.providers.generator.type}, fixer=${inspection.effectiveRules.providers.fixer.type}`);
    console.log(`- routing: enabled=${inspection.effectiveRules.routing?.enabled !== false}, default_profile=${inspection.effectiveRules.routing?.default_profile ?? "(unset)"}, planning_profile=${inspection.routing.profile}`);
    console.log(`- memory: enabled=${inspection.effectiveRules.memory?.enabled !== false}, backend=${inspection.effectiveRules.memory?.backend ?? "(unset)"}`);
    console.log(`- vector search: enabled=${inspection.effectiveRules.vector_search?.enabled === true}, data_dir=${inspection.effectiveRules.vector_search?.data_dir ?? "(unset)"}, max_results=${inspection.effectiveRules.vector_search?.max_results ?? "(unset)"}, parser_mode=${inspection.effectiveRules.vector_search?.parsers?.mode ?? "auto"}`);
    console.log(`- tools: enabled=${inspection.effectiveRules.tools?.enabled !== false}, json_validation=${inspection.effectiveRules.tools?.json_validation !== false}`);
    console.log(`- prompts: directory=${inspection.effectiveRules.prompts?.directory || "(built-in)"}, examples_directory=${inspection.effectiveRules.prompts?.examples_directory || "(built-in)"}`);
    ...
    if (inspection.toolSummaries.length > 0) {
        ...
        for (const tool of inspection.toolSummaries) {
            console.log(`  - ${tool.name}: enabled=${tool.enabled}, source=${tool.source}, scope=${tool.scope ?? "full"}, sandbox=${tool.sandboxMode ?? "inherit"}, image=${tool.sandboxImage ?? "(default)"}, scoped_changed_files=${tool.scopedToChangedFiles === true}, cwd=${tool.workingDirectory ?? "."}, command=${tool.command ?? "(none)"}${tool.args && tool.args.length > 0 ? ` ${tool.args.join(" ")}` : ""}`);
        }
    }
}
export function printConfigUseResult(preset, configPath, inspection) {
    console.log("");
...
export function printDoctor(inspection, presets) {
    console.log("");
    console.log("Doctor");
    console.log(`- repo: ${inspection.repoRoot}`);
    console.log(`- global config: ${inspection.globalConfigPath ?? "(none)"}`);
    ...
    console.log(`- profile: ${inspection.profile ?? "(none)"}`);
    console.log(`- effective providers: planner=${inspection.effectiveRules.providers.planner.type}, reviewer=${inspection.effectiveRules.providers.reviewer.type}, generator=${inspection.effectiveRules.providers.generator.type}, fixer=${inspection.effectiveRules.providers.fixer.type}`);
    console.log(`- routing decision: stage=${inspection.routing.stage}, enabled=${inspection.routing.enabled}, profile=${inspection.routing.profile}, reason=${inspection.routing.reason}`);
    console.log(`- memory: enabled=${inspection.effectiveRules.memory?.enabled !== false}, backend=${inspection.effectiveRules.memory?.backend ?? "(unset)"}`);
    console.log(`- vector search: enabled=${inspection.effectiveRules.vector_search?.enabled === true}, data_dir=${inspection.effectiveRules.vector_search?.data_dir ?? "(unset)"}, max_results=${inspection.effectiveRules.vector_search?.max_results ?? "(unset)"}, parser_mode=${inspection.effectiveRules.vector_search?.parsers?.mode ?? "auto"}`);
    console.log(`- run budgets: duration=${inspection.effectiveRules.execution?.budgets?.max_duration_ms ?? "(disabled)"}ms, cost=${inspection.effectiveRules.execution?.budgets?.max_cost_units ?? "(disabled)"}`);
    console.log(`- prompts: directory=${inspection.effectiveRules.prompts?.directory || "(built-in)"}, examples_directory=${inspection.effectiveRules.prompts?.examples_directory || "(built-in)"}`);
    ...
    if (inspection.toolSummaries.length > 0) {
        console.log("- effective tool commands:");
        for (const tool of inspection.toolSummaries) {
            console.log(`  - ${tool.name}: ${tool.summary} [source=${tool.source}, scope=${tool.scope ?? "full"}, sandbox=${tool.sandboxMode ?? "inherit"}, image=${tool.sandboxImage ?? "(default)"}, scoped_changed_files=${tool.scopedToChangedFiles === true}]`);
        }
    }
...
export function printSetupCheck(result) {
    console.log("");
    ...
    console.log(`- effective providers: planner=${result.inspection.effectiveRules.providers.planner.type}, reviewer=${result.inspection.effectiveRules.providers.reviewer.type}, generator=${result.inspection.effectiveRules.providers.generator.type}, fixer=${result.inspection.effectiveRules.providers.fixer.type}`);
