...
export async function handleWorkCommand(command, options) {
    const { cwd, configPath, explicitGlobalConfigPath, ignoreProjectConfig, outputJson, savePath } = options;
    switch (command.kind) {
        case "work-create": {
        ...
        case "work-from-pr": {
            ...
            if (!workItem)
                throw new Error(`Unsupported external task URL: ${command.url}`);
            if (outputJson) {
                await outputJsonResult(workItem, savePath);
                return true;
            }
        ...
        case "work-ci-fix": {
            ...
            const updated = { ...workItem, description: `${workItem.description}\n\nCI repair task: ${task}`, status: "running_checks", ci: { ...(workItem.ci ?? {}), ...report, repairAttempts: (workItem.ci?.repairAttempts ?? 0) + 1, lastCheckedAt: new Date().toISOString() }, updatedAt: new Date().toISOString() };
            await store.save(updated);
            if (outputJson) {
                await outputJsonResult(updated, savePath);
                return true;
