    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "work") {
            ...
            if (subCommand === "from-issue") {
                ...
                if (!url)
                    throw new Error("Missing URL for `ai work from-issue <url>`.");
            ...
            if (subCommand === "from-pr") {
                const url = args[index + 2];
                if (!url)
                    throw new Error("Missing URL for `ai work from-pr <url>`.");
                command = { kind: "work-from-pr", url };
                index += 2;
                continue;
            }
            if (subCommand === "inbox") {
                if (args[index + 2] !== "sync")
                    throw new Error("Use `ai work inbox sync`.");
        ...
        if (workerCommandActive && arg.startsWith("--")) {
            if (arg === "--server-url") {
                ...
                command = { ...command, serverUrl: nextArg };
                index += 1;
                continue;
            }
            if (arg === "--token") {
                ...
                command = { ...command, workerToken: nextArg };
                index += 1;
                continue;
            }
            if (arg === "--name") {
                ...
                command = { ...command, workerName: nextArg };
                index += 1;
                continue;
            }
            if (arg === "--labels") {
            ...
            if (arg === "--workspace-roots") {
                const nextArg = args[index + 1];
                if (!nextArg)
                    throw new Error("Missing value for `--workspace-roots`.");
                const workspaceRoots = nextArg.split(",").map((value) => value.trim()).filter(Boolean);
                command = { ...command, workspaceRoots };
                index += 1;
                continue;
            }
            if (arg === "--heartbeat-interval") {
                ...
                command = { ...command, heartbeatIntervalMs };
                index += 1;
                continue;
            }
            if (arg === "--poll-interval") {
                const nextArg = args[index + 1];
                if (!nextArg)
                    throw new Error("Missing value for `--poll-interval`.");
                const pollIntervalMs = Number(nextArg);
                if (!Number.isFinite(pollIntervalMs) || pollIntervalMs <= 0) {
                    throw new Error("`--poll-interval` must be a positive number.");
                }
                command = { ...command, pollIntervalMs };
                index += 1;
                continue;
            }
            if (arg === "--once") {
                command = { ...command, once: true };
                continue;
            }
...
export function normalizeRetryStage(value) {
    ...
    switch (normalized) {
        ...
        case "store-memory":
            return "memory-store";
        default:
            throw new Error("Unsupported retry stage. Use one of: context, generating, checking, reviewing, fixing, writing, memory.");
    }
}
