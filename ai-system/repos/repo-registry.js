...
export function resolveRepoRegistryPath(defaultCwd) {
...
export class RepoRegistryStore {
    ...
    async register(input, allowedRoots) {
    ...
    async load() {
        try {
            const parsed = JSON.parse(await fs.readFile(resolveRepoRegistryPath(this.defaultCwd), "utf8"));
            return {
                version: 1,
...
export async function resolveRegisteredRepoPath(defaultCwd, repoId, allowedRoots) {
...
function normalizeRepoId(value, localPath) {
