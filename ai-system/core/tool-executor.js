...
export async function runToolChecks({ repoRoot, changedFiles, rules, logger, signal }) {
    if (signal?.aborted)
    ...
    const toolNames = Object.keys(tools.commands || {});
    for (const standardTool of ["lint", "typecheck", "build", "test"]) {
        if (!toolNames.includes(standardTool))
            toolNames.push(standardTool);
    }
    for (const toolName of toolNames) {
        ...
        try {
            ...
            const failureClass = classifyToolFailure(error, resolved.sandboxMode);
            // B2: scope fallback — if scoped check failed, retry with full scope
            let fallbackResult = failureClass;
...
export async function summarizeConfiguredTools({ repoRoot, rules }) {
    const tools = rules.tools ?? {};
    const packageJson = await readPackageJson(path.join(repoRoot, "package.json"));
    const packageScripts = normalizeScripts(packageJson);
    const packageManager = await detectPackageManager(repoRoot);
...
export async function createDryRunToolExecutionSummary({ repoRoot, rules, reason = "Skipped command-based tool checks in dry-run mode because the isolated repo context is incomplete." }) {
    ...
    return {
        results: summaries.map((tool) => ({
            ...
            issueCount: 0,
            durationMs: 0,
            summary: tool.enabled === false || tool.source === "disabled"
