...
export async function readContextFiles(repoRoot, readFiles, rules, logger) {
    ...
    if (skippedFiles.length > 0) {
        logger?.warn(`Skipped ${skippedFiles.length} low-priority context file(s) because max_context_bytes=${rules.max_context_bytes} was reached.`);
    }
...
export async function filterExistingSafeReadFiles(repoRoot, requestedFiles, rules, logger, maxCount = rules.max_files) {
    const allowed = [];
