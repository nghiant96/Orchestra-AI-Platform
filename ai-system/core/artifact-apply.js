...
export async function applyArtifactCandidate({ repoRoot, configPath, target, dryRun, force, logger }) {
    ...
    if (!force && hasBlockingIssues(artifact.issues)) {
        throw new Error(`Artifact has blocking review issues. Re-run with --force if you still want to apply it. Summary: ${artifact.reviewSummary || "no review summary"}`);
    }
    ...
    const applyEventPath = await persistApplyEvent(artifact.runPath, {
        version: 1,
        task: artifact.task,
        dryRun,
        force,
...
export async function loadArtifactCandidate(repoRoot, rules, target) {
...
async function resolveIterationPath(repoRoot, rules, target) {
