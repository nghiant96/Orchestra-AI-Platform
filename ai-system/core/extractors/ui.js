import { normalizeTask, requirementIssue } from "./base.js";
...
export class UIExtractor {
    ...
    validateCoverage(contracts, files) {
        ...
        for (const file of candidates) {
            if (contracts.some((contract) => contract.id === "event-feed-filter-no-horizontal-scroll")
                ...
                issues.push({
                    ...requirementIssue(file.path, "Event Feed filter still appears to rely on horizontal scrolling instead of a wrapping or grid layout.", "Replace horizontal overflow/nowrap filter rows with flex-wrap or a responsive grid so filter controls never require sideways scrolling."),
            ...
            if (contracts.some((contract) => contract.id === "event-feed-filter-counts") && !hasPerFilterCount(file.content)) {
                issues.push({
                    ...requirementIssue(file.path, "The task asks for a job count beside each filter label, but the generated filter UI does not expose per-filter counts.", "Compute counts per filter/status and render the count inside each filter option next to its label."),
            ...
            if (contracts.some((contract) => contract.id === "event-feed-filter-header-then-controls") && !hasHeaderThenFilterStructure(file.content)) {
                issues.push({
                    ...requirementIssue(file.path, "The task asks for a clearer Event Feed filter structure with a title/header above and filters below.", "Render the Event Feed title/summary in a header row and place the filter controls in a separate row underneath."),
                    verificationCommand: "npm run lint",
                    affectedFiles: [file.path]
                });
            }
...
function hasHeaderThenFilterStructure(content) {
