...
import { listProjectConfigPresets, loadJsonIfExists, resolveProjectConfigPath, writeJsonFile } from "../utils/config.js";
...
export async function inspectProjectConfiguration({ repoRoot, explicitConfigPath, explicitGlobalConfigPath, ignoreProjectConfig = false, task }) {
    await loadEnvironment(repoRoot);
    const { rules, configPath, projectConfig, profile, globalConfigPath, globalConfig, globalProfile } = await loadRules(repoRoot, explicitConfigPath, explicitGlobalConfigPath, ignoreProjectConfig);
    const effectiveRules = JSON.parse(JSON.stringify(rules));
...
export async function writeProjectPreset({ repoRoot, explicitConfigPath, explicitGlobalConfigPath, preset }) {
    const resolvedConfigPath = explicitGlobalConfigPath ??
        ((await resolveProjectConfigPath(repoRoot, explicitConfigPath)) ?? path.join(repoRoot, ".ai-system.json"));
    const currentConfig = (await loadJsonIfExists(resolvedConfigPath)) ?? {};
    const nextConfig = {
        ...currentConfig,
        profile: preset
    };
...
export async function applySetupChoices({ repoRoot, explicitConfigPath, explicitGlobalConfigPath, choices }) {
    const resolvedConfigPath = explicitGlobalConfigPath ??
        ((await resolveProjectConfigPath(repoRoot, explicitConfigPath)) ?? path.join(repoRoot, ".ai-system.json"));
    const currentConfig = (await loadJsonIfExists(resolvedConfigPath)) ?? {};
    const explicitProviders = Object.fromEntries(Object.entries(choices.providers)
    ...
    const nextConfig = {
        ...
        tools: {
            ...
            commands: buildToolCommandsConfig(choices.tools ?? {}, currentConfig.tools?.commands)
        }
    };
    delete nextConfig.profile;
    if (Object.keys(explicitProviders).length > 0) {
        nextConfig.providers = explicitProviders;
    }
    else {
        delete nextConfig.providers;
    }
    ...
    await upsertEnvFile(envPath, {
        AI_SYSTEM_OPENMEMORY_BASE_URL: choices.memoryBackend === "openmemory" ? choices.openMemoryBaseUrl?.trim() || "http://127.0.0.1:9080" : null,
        AI_SYSTEM_OPENMEMORY_API_KEY: choices.memoryBackend === "openmemory"
...
function buildToolCommandsConfig(tools, currentCommands) {
    const output = { ...(currentCommands ?? {}) };
    for (const toolName of ["lint", "typecheck", "build", "test"]) {
        ...
        if (selection.mode === "auto") {
            ...
            if (current && typeof current === "object" && !Array.isArray(current)) {
                const { script: _script, command: _command, args: _args, append_changed_files: _append, ...rest } = current;
                output[toolName] = {
                    ...rest,
                    enabled: true
                };
                ...
                output[toolName] = { enabled: true };
            }
            continue;
        }
        if (selection.mode === "disabled") {
            output[toolName] = { enabled: false };
            continue;
        }
        output[toolName] = {
            enabled: true,
...
export async function runSetupCheck({ repoRoot, explicitConfigPath, explicitGlobalConfigPath, ignoreProjectConfig = false }) {
    ...
    const openmemory = inspection.effectiveRules.memory?.enabled !== false && inspection.effectiveRules.memory?.backend === "openmemory"
        ? {
            enabled: true,
...
async function validateAllowedWorkdirs(value) {
    if (!value)
        return [];
...
async function checkDashboardBuildScript(repoRoot) {
    try {
...
export async function readEnvValues(repoRoot) {
    const envPath = path.join(repoRoot, ".env");
    try {
...
function collectActiveEnvOverrides() {
    const result = [];
...
function buildRecommendations({ configPath, projectConfig, profile, effectiveRules, activeEnvOverrides, runtime, server, dashboard }) {
...
async function upsertEnvFile(envPath, changes) {
    let raw = "";
    try {
...
async function probeOpenMemory(memory, kind) {
...
function buildOpenMemoryHeaders(memory) {
...
async function requestUrl({ url, method, headers, body, timeoutMs }) {
