...
import { getProjectConfigPreset, loadJsonIfExists, mergeConfig, resolveGlobalConfigPath, resolveProjectConfigPath, stripProjectConfigProfile } from "../utils/config.js";
import { buildRoutingDecision } from "./provider-router.js";
import { createRuntimeDependencies } from "./run-executor.js";
const PROVIDER_ROLES = ["planner", "reviewer", "generator", "fixer"];
const ROUTING_CONTROL_KEYS = ["timeout_ms", "retries", "base_delay_ms", "monitor_interval_ms", "temperature", "response_format"];
const DEFAULT_PROVIDER_COMMANDS = {
    "agy-cli": "agy",
    "claude-cli": "claude",
    "codex-cli": "codex"
};
export async function loadOrchestratorRuntime({ repoRoot, explicitConfigPath, logger, task }) {
...
export async function rerouteRuntimeForPlan({ repoRoot, rules, task, plan, logger }) {
...
export async function loadRules(repoRoot, explicitConfigPath, explicitGlobalConfigPath, ignoreProjectConfig = false, logger) {
    ...
    const projectConfig = configPath ? await loadJsonIfExists(configPath) : null;
    const preset = getProjectConfigPreset(projectConfig?.profile);
    const projectRules = stripProjectConfigProfile(projectConfig);
    let mergedRules = mergeConfig(mergeConfig(mergeConfig(mergeConfig(baseRules, globalPreset?.config ?? null), globalRules), preset?.config ?? null), projectRules);
...
async function loadOptionalGlobalConfig(globalConfigPath, explicitGlobalConfigPath, logger) {
    try {
        return await loadJsonIfExists(globalConfigPath);
    }
    catch (error) {
        if (explicitGlobalConfigPath || process.env.AI_SYSTEM_GLOBAL_CONFIG) {
            throw error;
        }
        logger?.warn(`Ignoring unreadable default global config at ${globalConfigPath}: ${error.message}`);
        return null;
    }
}
function buildPromptAllowedRoots(repoRoot, configPath, globalConfigPath) {
...
function resolvePromptBaseDir(repoRoot, projectRules, configPath, globalRules, globalConfigPath) {
...
export async function prepareRuntimeRules({ repoRoot, rules, task, stage, plan, logger }) {
...
export function applyDynamicProviderRouting(rules, decision, logger) {
    if (!decision.enabled) {
    ...
    logger?.info(`Dynamic provider routing selected ${decision.stage} profile "${decision.profile}" (${decision.reason})${appliedSummary ? ` -> ${appliedSummary}` : ""}.`);
    return nextDecision;
}
function applyGenericEnvOverrides(rules) {
...
function applyExplicitEnvOverrides(rules) {
    ...
    applyOpenAICompatibleOverride([rules.providers.planner, rules.providers.reviewer, rules.providers.generator, rules.providers.fixer], {
        baseUrl: process.env.AI_SYSTEM_BASE_URL ||
            process.env.AI_SYSTEM_OPENAI_BASE_URL ||
            process.env.AI_SYSTEM_9ROUTER_BASE_URL,
        apiKey: process.env.AI_SYSTEM_API_KEY ||
            process.env.AI_SYSTEM_OPENAI_API_KEY ||
            process.env.AI_SYSTEM_9ROUTER_API_KEY,
        model: process.env.AI_SYSTEM_MODEL ||
...
function applyExplicitProviderTypeOverride(rules, role, providerType) {
...
function resolveProviderTemplate(rules, role, providerType) {
...
function normalizeAllProviderConfigs(rules) {
    const roles = ["planner", "reviewer", "generator", "fixer"];
...
function applyProviderTypeToAllRoles(rules, providerType) {
    const roles = ["planner", "reviewer", "generator", "fixer"];
    for (const role of roles) {
        rules.providers[role] = buildProviderConfigForType(rules, role, providerType);
    }
}
function buildProviderConfigForType(rules, role, providerType) {
...
function buildRoutedProviderConfig(original, template, providerType) {
    const routed = {
        ...template,
        type: providerType
    };
    const routedRecord = routed;
...
function hasExplicitRoleProviderOverride(role) {
    const envKey = role === "planner"
...
function isProjectLockedRole(rules, role) {
