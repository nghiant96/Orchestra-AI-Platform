...
export async function aggregateProjectStats(cwd, rules) {
    ...
    for (const run of recentRuns) {
        ...
        const waitTime = run.waitTimeMs || 0;
        const executionTime = run.executionTimeMs || 0;
        ...
        if (run.contracts) {
            for (const contract of run.contracts) {
                ...
                if (contract.status === "passed")
                    stats.contractStats.passedContracts += 1;
                if (contract.status === "failed")
                    stats.contractStats.failedContracts += 1;
    ...
    return {
        ...
        avgExecutionTimeMs: queueTiming.avgExecutionTimeMs || (stats.totalRuns > 0 ? Math.round(stats.totalExecutionTimeMs / stats.totalRuns) : 0),
...
async function loadQueueJobsForStats(cwd) {
...
function summarizeQueueTiming(jobs) {
    const waitTimes = jobs.map((job) => job.waitTimeMs).filter((value) => typeof value === "number");
...
function inferContractDomain(contract) {
    const id = contract.id || "";
    if (id.includes("ui") || id.includes("event-feed"))
        return "ui";
    if (id.includes("api") || id.includes("schema"))
        return "api";
    if (id.includes("security") || id.includes("dependency"))
        return "security";
    if (id.includes("test") || id.includes("risky"))
        return "tests";
    return "other";
}
