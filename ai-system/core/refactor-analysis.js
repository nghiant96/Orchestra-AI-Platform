 ...
 */
export async function performRefactorAnalysis({ repoRoot: _repoRoot, goal, changedFiles, dependencyGraph }) {
    const affectedFilesSet = new Set(changedFiles);
...
function buildDependencyClusters(files, graph) {
...
function identifyRiskAreas(changedFiles, graph) {
    const riskAreas = [];
...
function proposeRefactorBatches(clusters, _goal) {
    const batches = [];
    let batchCount = 0;
...
function createBatch(index, files, moduleName) {
...
function identifyRelatedTests(files) {
    const tests = new Set();
    for (const file of files) {
