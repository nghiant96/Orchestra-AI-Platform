...
export async function expandContextReadFiles({ repoRoot, rules, task, prompt, initialReadFiles, writeTargets, logger }) {
    const safeWriteTargetReads = await filterExistingSafeReadFiles(repoRoot, writeTargets ?? [], rules, logger, Math.max(writeTargets?.length ?? 0, rules.max_write_files ?? 8));
    ...
    if (rules.vector_search?.enabled) {
        ...
        vectorMatches = await vectorIndex.search([task, prompt, ...(initialReadFiles ?? [])].filter(Boolean).join("\n"), rules.vector_search?.max_results);
    }
    ...
    const mergedReadFiles = await filterExistingSafeReadFiles(repoRoot, rankedCandidates.map((entry) => entry.path), rules, logger, rankedCandidates.length);
...
export function rankContextCandidates({ initialReadFiles, dependencyFiles, writeTargetReads, changedHintFiles, vectorMatches }) {
    ...
    const upsert = (path, score, source) => {
...
function computeExpandedContextLimit({ rules, initialCount, writeTargetCount }) {
...
async function collectChangedWorkingTreeFiles(repoRoot, rules, logger) {
    try {
...
export async function trimRankedCandidatesByBudget({ repoRoot, rankedCandidates, maxExpandedFiles, maxContextBytes }) {
