...
import { detectSymbolRanges } from "./symbol-parsers.js";
...
export class VectorIndex {
    ...
    rules;
    logger;
    constructor({ repoRoot, rules, config, logger }) {
    ...
    async search(query, maxResults = this.config.max_results) {
...
async function collectCandidateFiles(repoRoot, rules, vectorDataDir, maxIndexedFiles) {
    const entries = [];
...
function isInternalIndexPath(relativePath, internalExcludedDirs) {
...
async function buildChunkRecords({ relativePath, content, chunkSize, chunkOverlap, parserConfig, logger }) {
    const chunks = await chunkText(relativePath, content, chunkSize, chunkOverlap, parserConfig, logger);
    const records = [];
    let index = 0;
    for (const chunk of chunks) {
        const embedding = await generateEmbedding([relativePath, chunk.symbolName ? `symbol ${chunk.symbolName}` : "", chunk.text].filter(Boolean).join("\n"));
        records.push({
...
async function chunkText(relativePath, content, chunkSize, chunkOverlap, parserConfig, logger) {
...
async function chunkTextBySymbols(relativePath, content, chunkSize, parserConfig, logger) {
    const text = String(content || "");
    if (!text.trim()) {
        return [];
    }
...
function chunkTextFixed(text, chunkSize, chunkOverlap) {
    const safeChunkSize = Math.max(chunkSize, 200);
...
function mergeAdjacentSmallChunks(chunks, chunkSize) {
    if (chunks.length <= 1) {
        return chunks;
    }
    const merged = [];
...
function scoreChunk(chunk, queryTokens, queryEmbedding) {
...
function scorePathWeight(relativePath) {
...
function snapshotMatchesConfig(snapshot, config) {
    if (!snapshot) {
        return false;
    }
    return (snapshot.config.chunkSize === config.chunk_size &&
