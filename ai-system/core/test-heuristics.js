 ...
 */
export function detectMissingTests({ changedFiles, blastRadius, toolResults }) {
    const missingTests = [];
...
function isTestCovered(testId, toolResults) {
    return toolResults.some(r => {
        if (!r.ok)
            return false;
        if (r.name === "test") {
            // If a specific test file was targeted
            if (r.args?.includes(testId))
                return true;
            // If it was a broad test run that likely included it
            if (r.summary.toLowerCase().includes("passed") && !r.args?.length)
                return true;
        }
        // Dashboard specific check
        if (testId === "dashboard-test" && r.name === "dashboard:test" && r.ok)
            return true;
        return false;
    });
}
