export function resolveApprovalPolicy(task, rules, targetPaths = [], context = {}) {
...
function collectRiskSignals(task, targetPaths, context) {
...
function classifyRisk(signals, riskScore) {
    if (signals.some((signal) => signal.severity === "blocked"))
        return "blocked";
    if (signals.some((signal) => signal.severity === "high") || riskScore >= 6)
        return "high";
    if (signals.some((signal) => signal.severity === "medium") || riskScore >= 3)
        return "medium";
    return "low";
}
function signalScore(severity) {
    if (severity === "blocked")
        return 10;
    if (severity === "high")
        return 5;
...
function buildPolicyReason(riskClass, skipApproval, signals) {
