...
export async function prepareFixChecksTask({ repoRoot, configPath, providerPreset, logger }) {
    ...
    const fileHints = await extractExistingFileHints(resolvedRepoRoot, failingChecks.flatMap((result) => [
        result.stdout ?? "",
        result.stderr ?? "",
        result.summary ?? "",
        result.command ?? "",
...
export function buildFixChecksTask(failingChecks, fileHints) {
    const lines = [
...
function truncateForTask(value, maxLength) {
