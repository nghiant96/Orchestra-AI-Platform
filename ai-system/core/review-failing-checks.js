...
export async function reviewFailingChecks({ repoRoot, configPath, providerPreset, logger }) {
    ...
    const fileHints = await extractExistingFileHints(resolvedRepoRoot, failingChecks.flatMap((result) => [result.stdout ?? "", result.stderr ?? "", result.command ?? "", ...(result.args ?? [])]));
    ...
    const reviewStep = await measureExecutionStep(executionSteps, "review-failing-checks", async () => await runtime.reviewer.reviewCode(reviewTask, null, false, originalFiles, candidateFiles, toolExecution.issues, diffSummaries, resolvedRepoRoot, ""), `Reviewed ${failingChecks.length} failing check(s) with ${runtime.reviewerProvider.id}.`);
...
async function readCandidateFiles(repoRoot, fileHints) {
    const files = [];
...
async function extractExistingFileHints(repoRoot, snippets) {
    const seen = new Set();
    const hints = [];
