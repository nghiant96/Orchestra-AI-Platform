...
import { PROFILE_NAMES, PROVIDER_ROLES, createRoleProviders, getRoutingProfile, normalizeProviderType } from "./provider-router-utils.js";
...
export async function buildAdaptiveRoutingInsights({ repoRoot, rules, routing, task, stage, plan }) {
    const adaptive = {
...
function buildAdaptiveRoleStats(runs, adaptive) {
    const stats = {};
...
function buildAdaptiveProfileScores(rules, stage, roleStats, adaptive) {
    const weights = {
    ...
    const roles = stage === "planning" ? ["planner"] : ["reviewer", "generator", "fixer"];
    const scores = {};
...
function createAdaptiveProfileSignal(category, runsConsidered, profileScores) {
...
async function loadAdaptiveRunHistory(repoRoot, rules, lookbackRuns) {
    ...
    for (const runDir of runDirs) {
        try {
            ...
            const failureClass = parsed.execution?.failure?.class ??
            ...
            const providerMetrics = Object.fromEntries((parsed.execution?.providerMetrics ?? [])
