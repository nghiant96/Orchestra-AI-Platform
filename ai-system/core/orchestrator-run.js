...
import { buildStoppedResult, createArtifactState, persistPlanArtifacts, persistRoutingArtifacts, persistRunState } from "./artifacts.js";
...
import { collectProviderUsageMetrics, createExecutionStateMachine, executeGenerationLoop, finalizeErroredRun, finalizeFailedRun, loadImplementationMemoryContext, readAndPersistContext } from "./run-executor.js";
...
export async function runOrchestrator(host, task, { dryRun = false, interactive = false, pauseAfterPlan = false, pauseAfterGenerate = false, approvalPolicy = null, externalTask = null, workflowMode = "standard", signal } = {}) {
    if (signal?.aborted)
    ...
    await executionMachine.runStage("routing-planning", async () => {
        await persistRoutingArtifacts(artifactState, {
            stage: routing.stage,
            task,
            decision: routing
        }, host.logger);
    ...
    const planningMemoryStep = await executionMachine.runStage("planning-memory", async () => await safelySearchMemory(runtime.memory, { task, stage: "planning" }, host.logger), { detail: "Loaded planning memories." });
    ...
    const plannerStep = await executionMachine.runStage("planner", async () => await runtime.planner.planTask(task, treeString, repoRoot, planningMemoryContext), { detail: `Planner provider: ${runtime.plannerProvider.id}.` });
    ...
    const contextExpansionStep = await executionMachine.runStage("context-expansion", async () => await expandContextReadFiles({
        repoRoot,
        rules,
        task,
    ...
    await persistPlanArtifacts(artifactState, {
        task,
        rawPlan,
        plan,
    ...
    const implementationRoutingStep = await executionMachine.runStage("routing-implementation", async () => await rerouteRuntimeForPlan({
        repoRoot,
        rules,
        task,
        plan,
    ...
    const localConfirmCheckpoint = (message, artifactPath) => host.confirmationHandler
    ...
    if (effectivePauseAfterPlan) {
        const confirmed = await localConfirmCheckpoint("Plan checkpoint saved. Review the plan artifact before continuing?", artifactState.stepPaths.plan);
        if (!confirmed) {
    ...
    const localConfirmPlan = (p) => host.confirmationHandler ? host.confirmationHandler.confirmPlan(p) : confirmPlan(p, signal);
    ...
    try {
        const implementationMemoryStep = await executionMachine.runStage("implementation-memory", async () => await loadImplementationMemoryContext(runRuntime.memory, task, plan, memoryStats, host.logger), { detail: "Loaded implementation memories." });
        const implementationMemoryContext = implementationMemoryStep.result;
        const contextStep = await executionMachine.runStage("context", async () => await readAndPersistContext(repoRoot, plan, rules, artifactState, host.logger), { detail: `Read ${plan.readFiles.length} context file(s).` });
        let { contextFiles } = contextStep.result;
        skippedFiles = contextStep.result.skippedFiles;
        if (effectivePauseAfterPlan) {
            const confirmed = await localConfirmCheckpoint("Context checkpoint saved. Edit context.json or files/ before generation if needed.", artifactState.stepPaths.context);
            if (!confirmed) {
...
function createRetryHintFromState(state, error) {
...
function createIterationLimitRetryHint(state) {
