 ...
 */
export function normalizePersistedRunState(raw) {
    const state = {
 ...
 */
export function normalizeQueueJob(raw) {
    return {
        ...raw,
        version: raw.version ?? 1,
        ...
        workerLogs: Array.isArray(raw.workerLogs)
            ? raw.workerLogs.filter((entry) => typeof entry === "string")
            : undefined,
...
function normalizeFailureClass(failureClass) {
    const mapping = {
