function createTransition(stage, status, detail, durationMs, iteration, metadata) {
    return {
        stage,
        status,
...
function isTerminalTransitionStatus(status) {
...
function deriveStepsFromTransitions(transitions) {
    return transitions
        .filter((transition) => isTerminalTransitionStatus(transition.status))
        .map((transition) => ({
        name: typeof transition.iteration === "number" && transition.stage.startsWith("iteration-")
...
export class ExecutionStateMachine {
    ...
    currentStage;
    onTransition;
    constructor({ summary, onTransition } = {}) {
    ...
    async enterStage(stage, { detail, iteration, metadata } = {}) {
    ...
    async finishStage(stage, status, { durationMs, detail, iteration, metadata } = {}) {
        if (this.currentStage && this.currentStage !== stage) {
