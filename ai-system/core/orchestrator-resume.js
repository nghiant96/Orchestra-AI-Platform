...
import { loadSavedContextArtifacts, persistRoutingArtifacts, resolveResumeStatePath, restoreArtifactState } from "./artifacts.js";
...
import { createExecutionStateMachine, executeGenerationLoop, finalizeFailedRun, finalizeSuccessfulRun, loadImplementationMemoryContext, readAndPersistContext } from "./run-executor.js";
...
export async function resumeOrchestrator(host, resumeTarget, options = {}) {
    ...
    let skippedFiles = saved.skippedContextFiles ?? [];
    let iterationResults = Array.isArray(saved.iterations) ? [...saved.iterations] : [];
    let currentResult = saved.result ?? null;
    let acceptedIssues = Array.isArray(saved.finalIssues) ? saved.finalIssues : [];
    ...
    const implementationMemoryStep = await executionMachine.runStage("implementation-memory", async () => await loadImplementationMemoryContext(runtime.memory, task, plan, memoryStats, host.logger), { detail: "Loaded implementation memories during resume." });
    const implementationMemoryContext = implementationMemoryStep.result;
    const contextRestoreStep = await executionMachine.runStage("context-restore", async () => await loadSavedContextArtifacts(artifactState, plan.readFiles ?? []), { detail: "Loaded saved context artifacts for resume." });
    let contextFiles = contextRestoreStep.result;
    if (saved.status === "paused_after_plan" && contextFiles.length === 0) {
        const contextResultStep = await executionMachine.runStage("context", async () => await readAndPersistContext(repoRoot, plan, rules, artifactState, host.logger), { detail: `Read ${plan.readFiles.length} context file(s) during resume.` });
...
function isResumableRunStatus(status, retryHint, forcedStage = null) {
...
function resolveResumeStrategy(saved, currentResult, acceptedIssues, iterationResults, forcedStage) {
...
function createForcedRetryHint(stage, currentResult, iterationResults) {
