...
import { buildDiffSummaries, hasBlockingIssues, mergeIssues, normalizeReviewResult, validateCandidateFiles } from "./reviewer.js";
import { buildStoppedResult, persistIterationArtifacts, persistRunState } from "./artifacts.js";
...
import { collectProviderUsageMetrics, getExecutionBudgetSummary, createBudgetRetryHint, sanitizeGeneratedFiles, findMissingPlannedWriteTargets, buildIncompleteGenerationIssue, shouldUseStrictReview } from "./run-executor-utils.js";
import { finalizeFailedRun, finalizeSuccessfulRun } from "./run-executor-finalize.js";
import { generateCandidate } from "./run-executor-steps.js";
export async function executeGenerationLoop({ startIteration, task, dryRun, pauseAfterPlan, pauseAfterGenerate, repoRoot, configPath, plan, skippedFiles, implementationMemoryContext, runtime, memoryStats, artifactState, initialState, contextFiles, rules, logger, confirmCheckpoint, resumeFromStage, successResultStatus, successPersistedStatus, budgetConfig, approvalPolicy = null, externalTask = null, externalUpdatePreviews = [], toolChecks = runToolChecks, signal }) {
    if (signal?.aborted)
    ...
    for (let iteration = startIteration; iteration <= rules.max_iterations; iteration += 1) {
        ...
        if (preIterationBudget?.exceeded) {
            const result = await finalizeFailedRun({
                ...
                artifactState,
                state,
                retryHint: createBudgetRetryHint(state, preIterationBudget, startIteration) ?? undefined,
                resultStatus: "failed",
        ...
        if (!shouldSkipGeneration) {
            ...
            const generation = await state.executionMachine.runStage(generationStage, async () => await generateCandidate({
                iteration,
                task,
                plan,
        ...
        if (missingWriteTargets.length > 0) {
            ...
            const artifactInfo = await persistIterationArtifacts(artifactState, {
                iteration,
                task,
                dryRun,
                plan,
        ...
        const toolExecution = await state.executionMachine.runStage("iteration-tools", async () => {
            if (!dryRun) {
        ...
        const reviewStage = await state.executionMachine.runStage("iteration-review", async () => normalizeReviewResult(await runtime.reviewer.reviewCode(task, plan, shouldUseStrictReview(approvalPolicy), originalFiles, state.currentResult.files, preReviewIssues, state.diffSummaries, repoRoot, implementationMemoryContext, blastRadius)), {
            iteration,
        ...
        if (postIterationBudget?.exceeded) {
            const result = await finalizeFailedRun({
                ...
                artifactState,
                state,
                retryHint: createBudgetRetryHint(state, postIterationBudget, iteration + 1) ?? undefined,
                resultStatus: "failed",
                persistedStatus: "failed",
                budgetConfig,
                approvalPolicy,
        ...
        if (pauseAfterGenerate) {
            const confirmed = await confirmCheckpoint(`Generation checkpoint saved for iteration ${iteration}. Review the candidate files before continuing?`, artifactInfo?.iterationPath ?? artifactState.latestIterationPath);
            if (!confirmed) {
        ...
        if (!hasBlockingIssues(state.acceptedIssues)) {
            ...
            if (successBudget?.exceeded) {
                const result = await finalizeFailedRun({
                    ...
                    artifactState,
                    state,
                    retryHint: createBudgetRetryHint(state, successBudget, iteration) ?? undefined,
                    resultStatus: "failed",
                    persistedStatus: "failed",
                    budgetConfig,
                    approvalPolicy,
