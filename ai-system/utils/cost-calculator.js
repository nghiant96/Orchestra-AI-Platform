...
export const PROVIDER_DEFAULT_PRICING = {
...
export function estimateProviderCost({ role, provider, model, promptTokens, completionTokens }) {
    ...
    const estimatedCostUnits = Number(((prompt / 1_000_000) * pricing.input + (completion / 1_000_000) * pricing.output).toFixed(6));
    return {
        role,
        provider,
        model,
...
export function estimateRunCostFromPlan({ task, plan, contextFiles, providerSummary, rules }) {
    ...
    const totalCostUnits = Number(usageMetrics.reduce((total, metric) => total + Math.max(0, metric.estimatedCostUnits || 0), 0).toFixed(4));
    return {
        usageMetrics,
        budget: {
            maxDurationMs,
