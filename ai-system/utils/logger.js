...
function log(logger, level, message, writer = console.log) {
...
export function shouldUseDashboard({ enableDashboard = true, outputJson = false, stdinIsTTY = Boolean(process.stdin.isTTY), stdoutIsTTY = Boolean(process.stdout.isTTY), term = process.env.TERM || "", disabled = process.env.AI_SYSTEM_DISABLE_TUI === "true" } = {}) {
...
export function createCliLogger({ verbose = true, enableDashboard = true, outputJson = false } = {}) {
    ...
    const logger = {
        ...
        dashboard(snapshot) {
            ...
            if (snapshot.diffSummaries) {
                latestDiffLines = snapshot.diffSummaries.map((entry) => `${entry.path}: +${entry.addedLines} -${entry.removedLines} ~${entry.changedLineEstimate} (${entry.beforeLineCount}->${entry.afterLineCount})`);
            }
...
function formatDashboardMetrics(counts, providerMetrics, budget) {
    const totalCost = providerMetrics.reduce((total, metric) => total + Math.max(0, metric.estimatedCostUnits || 0), 0);
    const providerLines = providerMetrics.length === 0
...
function formatStateTree(stageStatuses) {
...
function formatDiffPanel(diffLines, artifactPath) {
    const lines = [
