export function resolveTokenRole(config, headerValue) {
...
export function canAccessRoute(role, route, method = "GET") {
    ...
    if (role === "worker") {
        if (normalizedMethod === "POST" && route === "/workers")
            return true;
        if (normalizedMethod === "POST" && /^\/workers\/[^/]+\/heartbeat$/.test(route))
            return true;
        if (normalizedMethod === "POST" && /^\/workers\/[^/]+\/jobs\/claim$/.test(route))
            return true;
