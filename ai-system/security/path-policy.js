...
export async function isPathAllowed(candidate, allowedRoots) {
...
export function isForbiddenPath(filePath) {
    ...
    if (normalized.includes(`${path.sep}.ssh${path.sep}`))
        return true;
    if (normalized.includes(`${path.sep}.aws${path.sep}`))
        return true;
    if (normalized.includes(`${path.sep}.config${path.sep}gcloud${path.sep}`))
        return true;
    return false;
}
