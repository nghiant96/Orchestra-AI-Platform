...
export class ReviewerAgent {
    ...
    async reviewCode(task, plan, isStrict, originalFiles, candidateFiles, validationIssues, diffSummaries, cwd, memoryContext = "", blastRadius) {
    ...
    validateOutput(result, originalFiles, candidateFiles) {
        ...
        const filteredIssues = result.issues.filter((issue) => {
            if (!issue.path)
                return true; // Global issues are ok
        ...
        const validationNotes = droppedIssues.length === 0
            ? []
            : [
                {
                    severity: "low",
