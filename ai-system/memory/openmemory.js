...
export class OpenMemoryAdapter {
    ...
    async searchRelevant({ task, stage, plan }) {
        ...
        if (this.transport === "http") {
            ...
            const fallback = await this.requestJson(`/memory/all?user_id=${encodeURIComponent(this.userId)}&l=${encodeURIComponent(String(this.config.fallback_scan_limit ?? 50))}`, {
                method: "GET",
                timeoutMs: this.config.query_timeout_ms ?? 15000
            });
            const fallbackItems = (fallback && typeof fallback === "object" ? fallback.items : []) ?? [];
