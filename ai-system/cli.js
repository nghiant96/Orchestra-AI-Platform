...
import { printHelp, printResult, outputJsonResult } from "./cli/formatters.js";
...
async function main() {
    ...
    const { cwd, dryRun, chat, interactive, pauseAfterPlan, pauseAfterGenerate, help, configPath, globalConfig, providerPreset, resumeTarget, command, outputJson, savePath, workflowMode, retryStage, reviewStaged, reviewBase, reviewFailingChecks, reviewFiles, force, task } = options;
    if (help) {
        printHelp();
        return;
    }
...
async function runCliCommand(command, options) {
    if (await handleConfigCommand(command, options))
        return;
    if (await handleRunsCommand(command, options))
        return;
    if (await handleFixCommand(command, options))
        return;
    if (await handleReviewCommand(command, options))
        return;
    if (await handleWorkCommand(command, options))
        return;
    if (await handleWorkerCommand(command, options))
        return;
