...
export class OpenAICompatibleProvider {
    ...
    async runJson({ cwd: _cwd, label, systemPrompt, prompt, schema, timeoutMs, retries, baseDelayMs, signal }) {
    ...
    async requestJson({ systemPrompt, prompt, schema, timeoutMs, signal }) {
...
function buildCombinedSignal(timeoutMs, externalSignal) {
...
function isRetryableHttpError(error) {
    ...
    return ["timeout", "temporarily unavailable", "rate limit", "try again", "overloaded", "503", "429"].some((needle) => message.includes(needle));
}
function sleep(ms) {
