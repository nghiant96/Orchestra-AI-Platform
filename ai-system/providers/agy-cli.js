...
export class AgyCliProvider {
    ...
    async runJson({ cwd, label, systemPrompt, prompt, schema, timeoutMs, retries, baseDelayMs, signal }) {
...
function buildMonitorHandler(logger, label, providerId) {
    ...
    return ({ elapsedMs, monitorId }) => {
        logger.info(`${label} is still running via ${providerId} after ${formatDuration(elapsedMs)}${monitorId > 1 ? ` (heartbeat ${monitorId})` : ""}`);
    };
}
