...
export async function cleanupFinishedWorktree(repoRoot, workItem) {
    if (!workItem.worktreePath)
        return false;
    if (workItem.status !== "done" && workItem.status !== "cancelled" && workItem.status !== "failed")
        return false;
    try {
...
export async function pruneStaleWorktreeRoots(repoRoot, maxAgeDays) {
...
export async function cleanupWorkspaceLifecycle(repoRoot, rules) {
