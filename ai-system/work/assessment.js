...
export function assessWorkItem(workItem, rules) {
...
function determineComplexity(risk, signals, taskText) {
    if (risk === "blocked" || signals.some((signal) => signal.severity === "high"))
        return "large";
    if (risk === "high")
        return "large";
    if (taskText.length > 180 || signals.some((signal) => signal.severity === "medium"))
        return "medium";
    return "small";
}
function inferAffectedAreas(taskText, signals) {
    const areas = new Set();
    ...
    if (normalized.includes("test") || normalized.includes("spec"))
        areas.add("tests");
    if (normalized.includes("dashboard") || normalized.includes("ui") || normalized.includes("frontend"))
        areas.add("dashboard");
    for (const signal of signals) {
        if (signal.name === "dependency-change")
            areas.add("dependencies");
        if (signal.name === "critical-path")
            areas.add("core-workflow");
        if (signal.name === "contract-or-config")
            areas.add("contracts");
    }
    return [...areas];
}
