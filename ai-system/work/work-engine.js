...
export class WorkEngine {
    ...
    async createNodeExecutionRequests(workItem, options = { dryRun: true }) {
    ...
    attachQueuedRuns(workItem, assignments) {
        ...
        const graph = workItem.graph
            ? {
                ...
                nodes: workItem.graph.nodes.map((node) => {
                    ...
                    return runId ? { ...node, status: "running", assignedRunId: runId } : node;
                })
            }
            : workItem.graph;
    ...
    reconcileRunResults(workItem, jobs) {
        if (!workItem.graph || jobs.length === 0)
            return workItem;
        const jobById = new Map(jobs.map((job) => [job.jobId, job]));
        const graph = {
            ...workItem.graph,
            nodes: workItem.graph.nodes.map((node) => {
                if (!node.assignedRunId)
                    return node;
     ...
     */
    async handoffToPR(repoRoot, workItem, options = {}) {
        let item = { ...workItem };
...
function selectExecutableNodes(workItem, nodeId, maxNodes) {
...
function workflowModeForNode(workItem, node) {
    if (node.kind === "review" || node.kind === "inspect" || node.kind === "check")
        return "review";
    if (node.kind === "ci_fix")
        return "fix";
    if (node.kind === "implement")
        return workItem.type === "refactor" ? "refactor" : "implement";
    if (node.kind === "test")
        return "standard";
    return workItem.expectedOutput === "pull_request" ? "review" : "standard";
}
function updateChecklistForAssignments(items, assignments) {
    const assignmentByNode = new Map(assignments.map((assignment) => [assignment.nodeId, assignment.runId]));
    return items.map((item) => {
        const runId = assignmentByNode.get(item.id);
        return runId ? { ...item, status: "doing", evidence: { type: "run", ref: runId } } : item;
    });
}
function updateChecklistForJobs(items, nodes) {
    ...
    return items.map((item) => {
        ...
        if (node.status === "completed")
            return { ...item, status: "passed", evidence: { type: "run", ref: node.assignedRunId } };
        if (node.status === "failed" || node.status === "skipped")
            return { ...item, status: "failed", evidence: { type: "run", ref: node.assignedRunId } };
        if (node.status === "running")
            return { ...item, status: "doing", evidence: { type: "run", ref: node.assignedRunId } };
        return item;
    });
}
function graphStatusFromJob(job) {
    ...
    if (job.status === "cancelled")
        return "skipped";
    if (job.status === "queued" || job.status === "assigned" || job.status === "running" || job.status === "waiting_for_approval" || job.status === "cancel_requested")
        return "running";
    return "pending";
}
function workStatusFromGraph(nodes) {
    if (nodes.some((node) => node.status === "failed"))
        return "failed";
    if (nodes.some((node) => node.status === "running"))
        return "executing";
    if (nodes.length > 0 && nodes.every((node) => node.status === "completed" || node.status === "skipped"))
        return "done";
    return "planning";
}
