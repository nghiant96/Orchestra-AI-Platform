 ...
 */
export async function pollGhPrChecks(repoRoot, prNumber) {
    try {
        ...
        const status = failing.length > 0 || cancelled.length > 0
            ? "failing"
            : pending.length > 0
                ? "unknown"
                : successful.length > 0
        ...
        if (successful.length > 0)
            summaryParts.push(`${successful.length} passing`);
        if (pending.length > 0)
            summaryParts.push(`${pending.length} pending`);
        if (failing.length > 0)
            summaryParts.push(`${failing.length} failing`);
        if (cancelled.length > 0)
            summaryParts.push(`${cancelled.length} cancelled`);
        return {
            status,
 ...
 */
export async function watchCiForWorkItem(workItem, repoRoot) {
    ...
    if (prNumber != null) {
        ...
        if (polled) {
            return {
                ...
                repairNeeded: polled.status === "failing" &&
 ...
 */
export function syncCiToWorkItem(workItem, polled) {
    return {
        ci: {
 ...
 */
export function proposeCiRepairTask(workItem, report) {
    const checks = report.failingChecks.length > 0
